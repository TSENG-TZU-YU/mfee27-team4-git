...
// 教學領域篩選
$fieldOrder = isset($_GET["fieldOrder"]) ? $_GET["fieldOrder"] : "";
switch ($fieldOrder) {
  case '琴鍵類音樂':
...
// $sql = "SELECT * FROM teacher WHERE $fieldOrder $sqlSearch valid=1 ORDER BY $orderType LIMIT $startPage, $perPage";
          ...
          <div class="row">
            <form action="teachers.php" method="get">
              ...
              <div class="row">
                ...
                <input class="col form-control me-3" type="text" name="search" onfocus="this.value=''" value="<?php if (isset($_GET["search"])) {
                                                                                                                echo $search . " 搜尋結果";
                                                                                                              } else {
                                                                                                                echo "請輸入搜尋關鍵字";
                                                                                                              } ?>">
          ...
          <div class="d-flex">
            ...
            <div class="me-4">
              <form action="teachers.php" method="get">
                ...
                <button class="btn  btn-khak ">
                  師資編號
                </button>
              </form>
            </div>
            <p class="my-auto me-2">篩選：</p>
            <div class="me-2">
              <!-- 教學領域篩選 -->
              <select class="col form-select" aria-label="Default select example" name="fieldOrder">
            ...
            <div class="me-2">
              <!-- 教授課程師資篩選 -->
              <form action="teachers.php" method="get">
                <select class="form-select" aria-label="Default select example" name="courseOrder">
                  <option selected value="">教授課程</option>
        ...
        <div aria-label="Page navigation example text-end" class="d-flex mt-4  justify-content-center">
          <ul class="pagination">
            ...
            <?php for ($i = 1; $i <= $totalPage; $i++) : ?>
              <li class="page-item"><a class="page-link <?php if ($page == $i) echo "active"; ?>" href="teachers.php?page=<?= $i ?>&search=<?= $search ?>&order=<?= $order ?>"><?= $i ?></a></li>
            <?php endfor; ?>
            <li class="page-item">
...
<div class="me-2 d-flex">
  <!-- 教授課程師資篩選 -->
  <select class="col form-select me-2" aria-label="Default select example" name="courseOrder">
    <option selected value="">教授課程</option>
    <?php foreach ($CourseProductRows as $row) : ?>
      <option value="<?= $row["course_name"] ?>"><?= $row["course_name"] ?></option>
    <?php endforeach; ?>
  </select>
  <a class="col btn btn-grey ">
    篩選
  </a>
</div>